...
if (magicJS.read(blackKey)) {
  ...
  blacklist = [];
  magicJS.write(blackKey, blacklist.join(';'));
}
  ...
  if (magicJS.isResponse) {
    switch (true) {
      ...
      case /^https:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\?/.test(magicJS.request.url):
        ...
        break;
      // 匹配story模式（应该是短视频模式），用于记录Story的aid
      ...
      case /^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/tab/.test(magicJS.request.url):
        try {
          /**
           * 普通|概念版 主页顶部tab
           * 39|442:直播 bilibili://live/home
           * 40|99 :推荐 bilibili://pegasus/promo
           * 41|100:热门 bilibili://pegasus/hottopic
          ...
          /** 
           * 普通|概念版 右上角按钮
           * 222|107 游戏中心 bilibili://game_center/home
          ...
          /** 普通|概念版
           * 177|102 首页 bilibili://main/home
           * ---|103 频道 bilibili://pegasus/home
           * 179|104 动态 bilibili://following/home
           * ---|105 消息 bilibili://link/im_home
           * 181|106 我的 bilibili://user_center
           */
      ...
      case /^https?:\/\/app\.bilibili\.com\/x\/v2\/account\/mine/.test(magicJS.request.url):
        try {
          let obj = JSON.parse(magicJS.response.body);
          /** 普通|概念版
           * 396|396 离线缓存 bilibili://user_center/download
           * 397|397 历史记录 bilibili://user_center/history
           * 398|398 我的收藏 bilibili://user_center/favourite
           * 399|399 稍后再看 bilibili://user_center/watch_later
           * 171|171 创作首页 bilibili://uper/homevc
           * 172|172 稿件管理 bilibili://uper/user_center/archive_list
           * 534|534 打卡挑战
           * ---|174 有奖活动
           * 402|--- 个性装扮
           * 404|430 我的钱包 bilibili://bilipay/mine_wallet
           * 622|431 会员购   bilibili://maill/mine?msource=mine
           * ---|432 直播中心 bilibili://user_center/live_center
           * 407|433 联系客服
           * 410|434 设置     bilibili://user_center/setting
           */
          const itemList = new Set([396, 397, 398, 399, 171, 172, 534, 8, 4, 428, 352, 1, 405, 402, 404, 544, 407, 410, 622, 425, 426, 427, 430, 431, 432, 433, 434]);
